...
The minimal system/tool-focused behavior above improves **reliable tool calling** but can reduce the quality of **user-facing or creatively generated text** (e.g. roleplay, summaries, explanations). **Context-aware mode** is now implemented: you can set **`ollamaContextMode: "auto"`** on the model so the runtime **infers per turn** whether to use minimal/tool-focused behavior (when tools are expected, e.g. edit file, update substrate) or richer text-generation behavior (e.g. "explain X", "summarize", open-ended chat). Manual settings still work: `ollamaContextMode: "minimal"` or `"full"` override inference. See [context-aware-system-behavior](context-aware-system-behavior.md) for the tech spec and [configuration-reference](configuration-reference.md) §4.15 for the config field.
