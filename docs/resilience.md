...
- **Resilience daemon (implemented):** `npm run resilience:daemon` — see Option C in §6. No dependency on the main server. When using `npm run start:watch`, the daemon is started automatically unless `RESILIENCE_DAEMON=0`.
