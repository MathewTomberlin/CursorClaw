...
**Note:** If the user interrupts while a heartbeat was about to send a proactive message, that in-progress message is **not** delivered (the turn is cancelled). The next heartbeat run will see the "interrupted" notice and can send a new proactive message if needed. To avoid duplicated or confusing output: (1) the continuation prompt explicitly instructs the agent not to repeat or summarize what it had written before the interrupt—only report new progress, and not to add a closing summary or wrap-up paragraph; (2) the proactive message from a continuation run is deduped (trailing suffix, duplicate paragraph removal, **summary-paragraph dedup**, and **all-but-first summary** so a rephrased summary in the middle or at the end is removed) before delivery; (3) any heartbeat run that does not complete with HEARTBEAT_OK (e.g. continuation interrupted again, user message cancelled the run, or timeout) does not have its partial response delivered—we send a short notice ("Heartbeat was interrupted; no update this time.") and set that as the pending proactive message so the UI replaces the previous message and the user does not see a truncated summary. The runtime also applies the same deduplication (including summary-paragraph removal for heartbeat sessions) to assistant text at turn end, and deduplication during streaming (e.g. duplicated trailing suffix removal) to reduce duplication from adapter behavior.
