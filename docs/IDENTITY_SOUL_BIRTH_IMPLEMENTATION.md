...
| **HEARTBEAT.md** | Per-tick checklist for heartbeat turns. | **Keep empty (or comments only) to skip heartbeat API calls.** Add tasks when you want the agent to check something periodically. |
...
| **HEARTBEAT.md** | Per-tick checklist (already implemented). Empty or comments-only → skip heartbeat API (per template). | Every heartbeat turn. | Heartbeat user message (prepended to `heartbeat.prompt`). |
