...
- [Provider and model support](provider-model-support.md) — at-a-glance table of all providers (Ollama, cursor-agent-cli, openai-compatible, lm-studio, etc.).
...
- [LM Studio implementation guide](lm-studio-implementation-guide.md) — for operators who prefer LM Studio over Ollama (local OpenAI-compatible server). To validate an LM Studio model: `npm run validate-model -- --modelId=<id> --fullSuite` (see that guide).
