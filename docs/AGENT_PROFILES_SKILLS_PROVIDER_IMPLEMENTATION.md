...
- [x] **P.4.3** Model selection: from config list or from provider’s model list if API supports it.
  - [x] **P.4.3.1** RPC `provider.models.list`: params `{ providerId, profileId? }`; for `ollama` call `GET {baseURL}/api/tags` and return `{ models: { id: string, name?: string }[] }`; for `openai-compatible` call `GET {baseURL}/models` (baseURL may include /v1) with profile-resolved API key (Bearer), return same shape; for `cursor-agent-cli` return config-only model ids for that provider. Errors (e.g. network, 401) return structured error in result (`result.error`); never log API keys.
  - [x] **P.4.3.2** Config/UI: allow user to pick model from provider list when configuring a profile or model entry—e.g. “Refresh from provider” that calls `provider.models.list` and populates a dropdown; selected value remains a config `modelId` / `openaiModelId` so existing resolution unchanged. Optional: persist “last fetched” list in profile tmp for offline display.
  - **Success criteria:** (1) `provider.models.list` returns model list for ollama and openai-compatible when endpoint and (if needed) credentials are available. (2) Profile model selection still uses `config.models` and profile `modelId`; new RPC is for discovery only. (3) No credential or model list in logs.
  - **Guardrails:** Same as P.4.2: keys only from credential store; do not log provider responses that might contain sensitive data; rate-limit or cache list calls if needed to avoid hammering provider APIs.
...
- **Version:** 1.1  
- **Status:** Complete for all items above # Roadmap. Agent Profiles (A.1–A.4), Provider/Model (P.1–P.5 including P.4.3.2), Agent Skills (S.1–S.5), and Shared Message View / Tailnet (V.1–V.4) implemented. P.4.3.2 done: Config page "Model selection" section with provider dropdown, "Refresh from provider" (calls provider.models.list), model dropdown, Save (config.patch profiles with modelId).  
...
- **Changelog (1.21):** P.4.3.1 implemented. `src/providers/list-models.ts`: listProviderModels(providerId, config, profileRoot) for ollama (GET /api/tags), openai-compatible (GET baseURL/models with Bearer), cursor-agent-cli (config-only model ids). Returns { ok, models } or { ok: false, error: { code, message } }; never logs API keys. Gateway RPC `provider.models.list` (params: providerId; profileId in context); result is either { models } or { error }. METHOD_SCOPES and handler; tests in tests/list-models.test.ts and gateway integration test.
- **Changelog (1.22):** P.4.3.2 implemented. Config page: "Model selection" section (per selected profile). Provider dropdown (cursor-agent-cli, ollama, openai-compatible), "Refresh from provider" button calls providerModelsList and populates model dropdown; user can pick a model and Save; config.patch({ profiles }) updates profile modelId. api.ts: providerModelsList(profileId, providerId), ProviderModelsListResult. No offline persistence of list (optional deferred).
