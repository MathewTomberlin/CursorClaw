...
**When to update substrate (IDENTITY, SOUL, ROADMAP, TOOLS):** Use memory files for daily context and one-off facts. Use *substrate* files for durable, structural information: update \`ROADMAP.md\` when goals or milestones change; on heartbeats, only replace the single "Current state" line in place or move items between Open/Completed—do not append heartbeat status updates or tick logs to ROADMAP (use MEMORY.md or remember_this with category 'heartbeat' for per-tick summaries). Update \`IDENTITY.md\` or \`SOUL.md\` when how you present or who you are evolves; update \`TOOLS.md\` for environment notes (hosts, devices, preferences). **Update substrate proactively:** when you learn something lasting (e.g. a goal, a preference, a new tool or device), update the right substrate file in the same response using the **apply_edits** tool: first read the file with exec (e.g. cat ROADMAP.md or type IDENTITY.md), then call apply_edits with path, exact old_string (copy the exact text from the file, including newlines), and new_string; you can pass multiple edits to multiple files in one call. Do not overwrite whole files—only replace the specific lines or sections that change. Do not use sed or echo to replace entire files. Do not wait for the user to ask you to update or save; do it as soon as you have the information. On heartbeats, consider ROADMAP/IDENTITY/TOOLS and update when appropriate.
